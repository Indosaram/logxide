...
def log_performance(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            duration = time.time() - start_time
            performance_logger.error(f'{func.__name__} failed after {duration:.3f}s: {str(e)}')
            raise
...
def metrics(request):
    ...
    try:
        ...
        avg_duration = ApiLog.objects.aggregate(avg_duration=Avg('duration'))['avg_duration'] or 0
        
        # Count by status code
        from django.db.models import Count
        status_counts = ApiLog.objects.values('status_code').annotate(count=Count('status_code'))
        ...
        app_logger.info(f'Metrics retrieved: {metrics_data["total_requests"]} total requests')
...
class Command(BaseCommand):
    ...
    def init_sample_data(self):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Sample data initialized')
        )

    def test_logging_performance(self, count):
        """Test LogXide logging performance."""
        self.stdout.write(f'Testing LogXide logging performance with {count} messages...')
...
if __name__ == '__main__':
    ...
    try:
        ...
        execute_from_command_line(['django_integration.py', 'runserver', '0.0.0.0:8000'])
