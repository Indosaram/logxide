...
def log_performance(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            duration = time.time() - start_time
            error_logger.error(f'{func.__name__} failed after {duration:.3f}s: {str(e)}')
            raise
...
def internal_error(error):
    """Handle 500 errors."""
    error_logger.error(f'500 Internal Server Error: {request.method} {request.path}', exc_info=True)
    db.session.rollback()
...
def get_metrics():
    ...
    try:
        ...
        app_logger.info(f'Metrics retrieved: {metrics["total_requests"]} total requests')
