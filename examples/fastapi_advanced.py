...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        websocket_logger.info(f'WebSocket connection established. Total connections: {len(self.active_connections)}')

    def disconnect(self, websocket: WebSocket):
        self.active_connections.remove(websocket)
        websocket_logger.info(f'WebSocket connection closed. Total connections: {len(self.active_connections)}')

    async def send_personal_message(self, message: str, websocket: WebSocket):
        await websocket.send_text(message)

    async def broadcast(self, message: str):
        websocket_logger.info(f'Broadcasting message to {len(self.active_connections)} connections')
        for connection in self.active_connections:
            try:
                await connection.send_text(message)
            except Exception as e:
                websocket_logger.error(f'Failed to send message to connection: {str(e)}')
