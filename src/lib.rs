...
use handler::{
    ConsoleHandler, FileHandler, Handler, NullHandler, PythonStreamHandler, RotatingFileHandler,
    StreamHandler,
};
...
fn logxide(_py: Python, m: &Bound<'_, pyo3::types::PyModule>) -> PyResult<()> {
    ...
    logging_module.add_function(wrap_pyfunction!(
        register_rotating_file_handler,
        &logging_module
    )?)?;
...
fn register_file_handler(_py: Python, filename: String, level: Option<u32>) -> PyResult<()> {
    use std::sync::Arc;
        ...
        let handler = match stream {
            ...
            Some(s) => {
                return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                    "Invalid stream '{}': must be 'stdout' or 'stderr'",
                    s
                )))
            }
        };
