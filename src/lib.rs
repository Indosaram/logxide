...
fn basic_config(_py: Python, kwargs: Option<&PyDict>) -> PyResult<()> {
    ...
    let root_level = if let Some(kw) = kwargs {
        if let Some(level) = kw.get_item("level") {
            let level: u32 = level.extract()?;
    ...
    let format_string = if let Some(kw) = kwargs {
        if let Some(format_val) = kw.get_item("format") {
    ...
    let date_format = if let Some(kw) = kwargs {
        kw.get_item("datefmt").map(|datefmt_val| {
            datefmt_val
                .extract::<String>()
...
fn register_python_handler(py: Python, handler: &PyAny) -> PyResult<()> {
    ...
    let py_handler = PythonHandler::new(handler.to_object(py));
    HANDLERS.lock().unwrap().push(Arc::new(py_handler));
    Ok(())
}
...
#[pymodule]
fn logxide(py: Python, m: &PyModule) -> PyResult<()> {
