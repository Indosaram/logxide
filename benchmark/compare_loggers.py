...
class LoggerBenchmark:
    ...
    def print_results(self):
        ...
        for benchmark_name, results in self.results.items():
            ...
            print("-"*80)
            print(f"{'Logger':<15} {'Avg Time (s)':<15} {'Ops/sec':<15} {'Std Dev':<15} {'Min (s)':<15} {'Max (s)':<15}")
            print("-"*80)
            ...
            if sorted_results:
                ...
                for logger_name, metrics in sorted_results:
                    ...
                    print(f"  {logger_name}: {ratio:.2f}x slower" if ratio > 1.01 else f"  {logger_name}: fastest")
